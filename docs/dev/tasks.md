...
Prepared: 2026-09-20–21. Status: execution backlog; no application session is complete yet.

This document turns [plan.md](plan.md) into **63 bounded sessions, S00–S62**. The default order is numerical. Dependency exceptions below let engineering proceed while clinical packages await owner review. Session IDs are stable: split an oversized session into `Sxx.a`, `Sxx.b` with explicit handoffs; do not renumber the backlog or silently skip acceptance criteria.

## 1. Instructions for every coding agent

### 1.1 Start here

1. Read workspace `AGENTS.md`, plan.md §§1–4 and §12, this execution contract, the selected session, and its named source files. Read current `docs/dev/progress-tracker.md`, relevant implementation, migrations, and existing tests. Inspect `git status`; preserve unrelated work.
...
3. State the session outcome, its approved seam IDs, and the first observable failing behavior. T1–T10 are already owner-approved in plan.md §12.1; do not ask again. If the interface actually changes, describe the change and ask before testing at a new seam.
...
6. Update progress and provide a concrete handoff. Do not commit, publish, deploy to a live host, or call a paid provider merely because a task ends. Those actions need the applicable existing authorization; local disposable verification is part of these tasks.

Use the requested [TDD](../../.agents/skills/tdd/SKILL.md), [system-design](../../.agents/skills/system-design/SKILL.md), and [codebase-design](../../.agents/skills/codebase-design/SKILL.md) instructions. Consult TDD's tests/mocking references. Other skills may help when actually applicable. The repository's agent role files are optional role descriptions, not mandatory orchestration. Do not assume `graphify`, `dev-test`, `verify`, or a task CLI exists. No session requires parallel agents.

### 1.2 Session limits and evidence
