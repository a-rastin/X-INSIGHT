...
2. `physician()` - 31 edges
3. `DraftEditor()` - 27 edges
4. `Appendix C.` - 27 edges
5. `_require_session()` - 25 edges
...
### Community 1 - "test_content.py"
Cohesion: 0.06
Nodes (23): alembic, alembic_config, _database_url(), Run migrations in 'offline' mode. This configures the context with just a URL…, Run migrations in 'online' mode. In this scenario we need to create an Engine…, run_migrations_offline(), run_migrations_online(), _isolate_audit_rows() (+15 more)
...
### Community 6 - "App.tsx"
Cohesion: 0.09
Nodes (24): react, react-dom, fetchSession(), listPhysicians(), App(), currentRoute(), DIAG_CORE, DIAG_DEFAULTS (+16 more)

### Community 7 - "app.py"
Cohesion: 0.08
Nodes (31): Attach the standard contract handlers to another app (tests reuse)., Propagate/generate request IDs, enforce body size, echo ID on errors., register_exception_handlers(), RequestContextMiddleware, Released-only assessment content serving., Patient registration (S06 slice 1: create patient + registration draft)., new_request_id(), parse_idempotency_key() (+23 more)
...
Cohesion: 0.11
Nodes (45): physician(), create_patient(), create_physician(), discard_audit_rows(), discard_headers(), draft_headers(), encounter_body(), login() (+37 more)

### Community 11 - "routes.py"
Cohesion: 0.15
Nodes (34): error_body(), Build the standard error envelope (plan section 4.3)., Compare an exact password against a stored hash; False on malformed., verify_password(), change_password(), _check_csrf(), _client_key(), _generic_login_denied() (+26 more)

### Community 12 - "accounts.py"
Cohesion: 0.16
Nodes (35): account_response(), AccountCreate, AccountEdit, change_active(), check_revision(), create_physician(), deactivate(), DeactivateRequest (+27 more)
...
Cohesion: 0.13
Nodes (18): create_patient(), _encounter_payload(), list_patients(), _patient_payload(), PatientCreate, Any, BaseModel, field_validator (+10 more)
...
Cohesion: 0.22
Nodes (9): 2.1 Set Up Project Structure, 2.2 Implement Core Infrastructure, 2.3 Implement Tools, 3.1 Code Quality, 3.2 Build and Test, 🚀 High-Level Workflow, Phase 2: Implementation, Phase 3: Review and Test (+1 more)
...
Cohesion: 0.11
Nodes (34): Clear all throttle state (tests only)., reset_all(), clean_diagnosis(), fixture, clean_drafts(), fixture, _audit_rows(), _clean_identity() (+26 more)
...
### Community 91 - "encounters.py"
Cohesion: 0.12
Nodes (31): _apply_diagnosis_ack(), _apply_panss_validation(), discard_encounter(), DiscardRequest, DraftPatch, _get_encounter(), list_encounters(), patch_encounter() (+23 more)
...
Cohesion: 0.15
Nodes (25): discardEncounter(), getEncounter(), diagAnswersFromStored(), DraftEditor(), buildDraftPayload(), handleClose(), handleDiagAnswers(), handleDiagBool() (+17 more)

### Community 103 - "api.ts"
Cohesion: 0.13
Nodes (22): 5. Persistence and invariants, createPatient(), createPhysician(), csrfToken(), Encounter, idempotencyKey(), listEncounters(), patchEncounter() (+14 more)

### Community 104 - "http/test_panss.py"
Cohesion: 0.11
Nodes (30): evaluate_panss(), Any, S10 slice 1 explicit-selection PANSS evaluator. Pure, no I/O, no DB., Evaluate PANSS answers. Delegates to generic evaluator., S10 slice 1: fresh PANSS form unanswered/skip (seam T2, no I/O, no DB).…, test_all_1_yields_7_7_16_30(), test_all_7_yields_49_49_112_210(), test_empty_answers_is_unanswered_with_null_scores() (+22 more)
...
### Community 140 - "panss.spec.ts"
Cohesion: 0.19
Nodes (13): createPatientViaPhysicianApi(), createPhysicianViaAdminApi(), dismissResearchNotice(), loginViaUI(), openDraftForPatient(), PANSS_ALL_30, PANSS_GENERAL, PANSS_NEGATIVE (+5 more)

### Community 141 - "Quick Reference"
Cohesion: 0.33
...
### Community 143 - "Security Best Practices"
Cohesion: 0.40
...
### Community 145 - "Phase 4: Create Evaluations"
Cohesion: 0.40
Nodes (5): 4.1 Understand Evaluation Purpose, 4.2 Create 10 Evaluation Questions, 4.3 Evaluation Requirements, 4.4 Output Format, Phase 4: Create Evaluations

### Community 146 - "content_hash"
Cohesion: 0.50
Nodes (5): canonical_json(), content_hash(), Any, Encode canonical UTF-8 JSON: sorted keys, compact, finite numbers. Object keys…, SHA-256 hex of the canonical JSON encoding.
...
### Community 149 - "Transport Options"
Cohesion: 0.50
Nodes (4): stdio, Streamable HTTP, Transport Options, Transport Selection

### Community 150 - "ReadinessError"
Cohesion: 0.50
Nodes (3): Exception, Database readiness failure with a public code., ReadinessError

### Community 151 - "login"
Cohesion: 0.67
Nodes (4): jsonOrThrow(), login(), LoginForm(), submit()

### Community 153 - "DiagnosisSection"
Cohesion: 0.50
Nodes (3): DiagnosisSection(), handleDiagAck(), previewDiagnosis()
...
## Knowledge Gaps
- **858 isolated node(s):** `x-insight`, `PANSS_POSITIVE`, `PANSS_NEGATIVE`, `PANSS_GENERAL`, `PANSS_ALL_30` (+853 more)
...
- **Why does `@playwright/test` connect `identity.spec.ts` to `panss.spec.ts`, `autosave.spec.ts`, `diagnosis.spec.ts`, `themes.spec.ts`, `accessibility.spec.ts`, `discard.spec.ts`, `registration.spec.ts`, `draft-safety.spec.ts`, `package.json`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `X-INSIGHT — System Design` connect `X-INSIGHT — System Design` to `X-INSIGHT — MCP Server Design`, `api.ts`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `react` connect `App.tsx` to `package.json`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Are the 23 inferred relationships involving `physician()` (e.g. with `test_answers_change_invalidates_prior_ack()` and `test_below_threshold_without_ack_stays_unacknowledged()`) actually correct?**
  _`physician()` has 23 INFERRED edges - model-reasoned connections that need verification._
- **What connects `x-insight`, `PANSS_POSITIVE`, `PANSS_NEGATIVE` to the rest of the system?**
...
- **Should `test_content.py` be split into smaller, more focused modules?**
